...

{
  ...
  eval {
    $jv->load_and_validate_schema($document);
  };
  ok !$@, 'local schema.json' or diag $@;

  is $jv->version, 7,     'detected version from local document, as draft-07';
  is $jv->_id_key, '$id', 'detected id_key from local document, draft-07';
}

{
  ...
  is $jv->_id_key, '$id', 'detected id_key from document via url, draft-07';
}
